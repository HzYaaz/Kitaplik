...
{
	kitaplar[kitapSayisi] = kitap;        //Kullanicinin girdigi kitaplari KitapSayisi olarak array içine yazdýk ve kitap nesnesine eþitledik.
	kitapSayisi++;      // Kullanici kitap girdikce , kitabý 1 arttýracak
}
...
{
	for (int i = 0; i < kitapSayisi; i++)     // For döngüsüyle , kullanicin girdiði kitaplarýn bilgilerini yazdýrýyoruz.
	{
...
{                                  
	Kitap k;                // Kitap.h'daki fonksiyonu çekmek için k nesnesi oluþturduk.
	bool dongu = true;
	...
	while (dongu)
	{
		...
		switch (cevap)          // kullanicinin verdigi cevaba yonelik sonuçlar..
		{
			...
			case 'D':
			{
				...
				dongu = false;   // kullanici 'D' ye basarsa dongu = false oluyor. Yani While dongusunden çýkýyor , program bitiyor.
				break;
			}
